...
static int rtsp_check_url (rtsp_client_t *client, const char *url)
{
    ...
    //mTRACE("server ip = %s port=%d url=%s\n", client->server_name, client->server_port, client->url);
...
int _rtsp_open(int handle, int serviceGroupID)
{
    ...
    /*setup-----------------------------------------------------*/
    memset(&cmd, 0, sizeof(cmd));
    cmd.transport = "MP2T/DVBC/QAM";
    cmd.serviceGroupID = serviceGroupID;
